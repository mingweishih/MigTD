...
impl ServtdCorim {
    ...
    /// `expected_signer_anchor` — the RTMR1-measured policy signer anchor
    /// derived from the CFV policy issuer chain (see [`compute_signer_anchor`]).
    /// This ties the CoRIM signer to the exact root-of-trust the firmware
    /// measured at boot, mirroring how the legacy JSON collateral is rooted in
    /// the measured policy issuer chain. Only after the anchor matches is the
    /// inner CoRIM payload (`bstr .cbor #6.501(corim-map)`) extracted and
    /// decoded via [`decode`](Self::decode).
...
/// Both documents now share `class = { vendor: "Intel", model: "TDX" }` and
/// `instance = #6.560("migration-td")`. A triple is in the migration-TD
/// environment iff both match.
